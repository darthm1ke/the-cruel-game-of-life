...
// Wait for the pixel font (or a 2.5s timeout) so static text uses it, not a
// fallback. No top-level await -> keeps the es2020 build target happy.
Promise.race([
  document.fonts.load('32px "VT323"').then(() => document.fonts.ready),
